				 ...
				 success:function(data)
						 {
							...
							{
								next_slide.find('[next],[prev]').parent().before(data);
								next_slide.attr('ajax-done','');
							}else
							{
				  ...
				  error:function(x,y,z)
				  {
					  console.log(x,y,z);
					  ajaxprocessing=false;
					  $this.appBusy({status:"hide",top:options.height/2,left:options.width/2});
				  }
			  });
...
$.fn.appBusy=function(options)
{
	...
	{//console.log(status);
		$('#jformslider_busy').css({ 'top' :options.top,'left':options.left }).show();
	}else if(options.tatus=="hide")
	{
